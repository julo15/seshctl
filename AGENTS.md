...
**Release pipeline.** `make dist` (unchanged) produces the signed DMG. New `make appcast` step mirrors the DMG into `dist/releases/` (gitignored), optionally renders `docs/release-notes/<VERSION>.md` for the appcast `<description>`, then runs Sparkle's `generate_appcast` over the mirror dir — emits a complete `docs/appcast.xml`. Commit + push to publish on Pages. The standard `gh release create v<VERSION> dist/Seshctl-<VERSION>.dmg --notes-file docs/release-notes/<VERSION>.md` finishes the release. The full flow lives in [`docs/release.md`](docs/release.md).
